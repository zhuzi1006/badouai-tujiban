#coding:utf8
# 不是很理解，修改，当字符串中出现a,c,e中的一个时，为正样本
import torch
...
class TorchModel(nn.Module):
    def __init__(self, input_dim, sentence_length, vocab):
        ...
        self.embedding = nn.Embedding(len(vocab) + 1, input_dim)  #Embedding层,将字符转化为矩阵。单独使用方法见embedding.py
        self.layer = nn.Linear(input_dim, input_dim)   #线性层 y = w*x+b,单独使用方法见DNNforward.py
        self.pool = nn.AvgPool1d(sentence_length)      #池化层 见ppt P20，此处为平均池化。
        self.classify = nn.Linear(input_dim, 1)        #用线性层进行分类
        self.activation = torch.sigmoid     #sigmoid做激活函数
        self.dropout = nn.Dropout(0.1)      #Dropout层，单独使用见Dropout.py
...
def build_sample(vocab, sentence_length):
    ...
    #指定哪些字出现时为正样本
    if set("ace") & set(x):    # 修改，当ace中一个出现在样本中，为正样本
        y = 1
    #指定字都未出现，则为负样本
    else:
        y = 0
