...
class TorchModel(nn.Module):
    ...
    def forward(self, x, y=None):
        ...
        x = self.layer(x)      #input shape:(batch_size, sen_len, input_dim)  (20,6,3)
        x = self.dropout(x)    #input shape:(batch_size, sen_len, input_dim)
        x = self.activation(x) #input shape:(batch_size, sen_len, input_dim)
        x = self.pool(x.transpose(1,2)).squeeze() #input shape:(batch_size, sen_len, input_dim)  (20,6,1)->(20,6)
...
if __name__ == "__main__":
    main()
    test_strings = ["aaaaaa", "casdfg", "rqweqg", "nlkdww"]
