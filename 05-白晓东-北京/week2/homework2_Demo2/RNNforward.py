...
#自定义RNN模型
class DiyModel:
    def __init__(self, w_ih, w_hh, w_bias_ih, w_bias_hh, hidden_size):
        self.w_ih = w_ih
        self.w_hh = w_hh
        self.w_bias_hh = w_bias_hh.detach().numpy()
        self.w_bias_ih = w_bias_ih.detach().numpy()
    ...
    def forward(self, x):
        ...
        for xt in x:
            ...
            ht_next = np.tanh(ux + self.w_bias_ih + wh + self.w_bias_hh)
...
w_bias_ih = torch_model.state_dict()["layer.bias_ih_l0"]
w_bias_hh = torch_model.state_dict()["layer.bias_hh_l0"]
#
...
print("---------------")
diy_model = DiyModel(w_ih, w_hh, w_bias_ih, w_bias_hh, hidden_size)
