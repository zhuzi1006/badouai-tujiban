...
class TorchModel(nn.Module):
    def __init__(self, input_dim, sentence_length, vocab):
        ...
        self.embedding = nn.Embedding(len(vocab) + 1, input_dim)  # 输入是字典的大小和输入的维度
        self.layer = nn.Linear(input_dim, input_dim)  # 线性层，输入维度和输出维度
        self.pool = nn.MaxPool1d(sentence_length)  # 1D的窗口大小
        self.classify = nn.Linear(input_dim, 3)  # 线性层 输出特征是3维
    ...
    def forward(self, x, y=None):
        x = self.embedding(x)   # 输入的shape为 20*6
        x = self.layer(x)       # 输入 20*6*20
        x = self.dropout(x)     # input shape:(batch_size, sen_len, input_dim) 20*6*20
        x = self.activation(x)  # input shape:(batch_size, sen_len, input_dim) 20*6*20
        x = self.pool(x.transpose(1, 2)).squeeze()  # input shape:(batch_size, sen_len, input_dim)
        x = self.classify(x)   # input shape:(batch_size, input_dim)  (20,20）
        y_pred = self.activation(x)               # input shape:(batch_size, 1) (20, 3)
        if y is not None:
