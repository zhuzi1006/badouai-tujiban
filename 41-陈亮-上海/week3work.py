...
class TorchModel(nn.Module):
    def __init__(self, input_dim, sentence_length, vocab):
        ...
        self.classify = nn.Linear(input_dim, 4)
        # self.activation = torch.sigmoid     #sigmoid做激活函数
        self.activation = torch.relu   #relu 做激活函数
        # self.activation = torch.nn.ReLU()  #relu 做激活函数
...
def build_sample(vocab, sentence_length):
    ...
    #B类样本
        ...
        y = 1
    #C类样本
    elif not set("abc" ) & set(x) and set("def") & set(x):
        y = 2
    #D类样本
    else:
        y = 3
...
def main():
    ...
    # optim = torch.optim.SGD(model.parameters(), lr=0.005)   #建立优化器,SGD优化器正确率太低了
    log = []
...
if __name__ == "__main__":
    main()
    # test_strings = ["abchij", "hijxyz", "dkfhij", "klmist"]
