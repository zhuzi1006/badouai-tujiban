...
def build_sample(vocab, sentence_length):
    ...
    #A类样本
    if set("abc") & set(x) and not set("efg") & set(x):
        y = 0
    #B类样本
    elif set("efg") & set(x) and not set("abc") & set(x):
        y = 1
    #指定字都未出现，则为负样本
    else:
        y = 2
...
def evaluate(model, vocab, sample_length):
    ...
    y = y.tolist()
    y = sum(y,[])
    print("A类样本数量：%d ，B类样本数量：%d ，C类样本数量：%d"%(y.count(0), y.count(1), y.count(2)))
    correct, wrong = 0, 0
    with torch.no_grad():
        y_pred = model(x)      #模型预测
...
if __name__ == "__main__":
    main()
    test_strings = ["abvxxx", "casdfg", "rqweqg", "nlkdww"]
