...
def build_sample(vocab, sentence_length):
    ...
    # 指定哪些字出现时为正样本
    if set("xyz") & set(x) and not set("abc") & set(x):
        y = 1
    # 指定字都未出现，则为负样本
    else:
        y = 0
...
def evaluate(model, vocab, sample_length):
    model.eval()
    x, y = build_dataset(1000, vocab, sample_length)  # 建立200个用于测试的样本
    print("本次预测集中共有%d个正样本，%d个负样本" % (sum(y), 1000 - sum(y)))
    correct, wrong = 0, 0
    with torch.no_grad():
        y_pred = model(x)  # 模型预测
...
def main():
    ...
    optim = torch.optim.Adam(model.parameters(), lr=0.05)  # 建立优化器
    log = []
...
if __name__ == "__main__":
    main()
    test_strings = ["axvaee", "xtztta", "rxweyg", "ylxdwz"]
