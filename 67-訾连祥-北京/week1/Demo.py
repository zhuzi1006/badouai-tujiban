...
class TorchModel(nn.Module):
    def __init__(self, input_dim, sentence_length, vocab):
        ...
        self.embedding = nn.Embedding(len(vocab) + 1, input_dim) #字符串转换成文本向量
...
#abc -> [1,2,3]
def build_vocab():
    file = open("./vocab.json",encoding='utf-8')
...
def build_sample(vocab, sentence_length):
    ...
    #指定字都未出现，则为负样本
        ...
        y = 0
    x = [vocab.get(word, vocab['某']) for word in x]   #将字转换成序号，为了做embedding
    return x, y
#建立数据集
#输入需要的样本数量。需要多少生成多少
...
def main():
    ...
    # sentence_length = 6  # 样本文本长度
    vocab = build_vocab()
    file = open("./param.json",encoding='utf-8')
    param = json.load(file)
    print('-----------配置参数----------------')
    print(param)
    model = build_model(vocab, param['char_dim'], param['sentence_length'])
    optim = torch.optim.Adam(model.parameters(), lr=0.005)  # 建立优化器
    log = []
    for epoch in range(param['epoch_num']):
        model.train()
        watch_loss = []
        for batch in range(int(param['train_sample'] / param['batch_size'])):
            x, y = build_dataset(param['batch_size'], vocab, param['sentence_length'])  # 构建一组训练样本
        ...
        acc = evaluate(model, vocab, param['sentence_length'])  # 测试本轮模型结果
...
if __name__ == "__main__":
    # main()
    test_strings = ["国", "钱", "周朝","李某","你叫什么","我叫某"]
