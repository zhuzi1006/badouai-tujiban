...
class TorchModel(nn.Module):
    def __init__(self, input_dim, sentence_length, vocab):
        ...
        #self.loss = nn.functional.softmax(vocab,dim=1)
...
#abc -> [1,2,3]
def build_vocab():
    chars = "abcdefghijklmnopqrstuvwxyz拼音输入法"  #字符集
    vocab = {}
    ...
    with torch.no_grad():  #不需要使用梯度
        ...
        for y_p, y_t in zip(y_pred, y):  #与真实标签进行对比
            if int(torch.argmax(y_p)) == int(y_t):
                ...
                wrong += 1
        total = correct + wrong
    print("总数：%d, 正确预测个数：%d / %d, 正确率：%f"%(total, correct, total, correct/(correct+wrong)))
    return correct/(correct+wrong)


def main():
    epoch_num = 15        #训练轮数
    batch_size = 26       #每次训练样本个数
    ...
    optim = torch.optim.Adam(model.parameters(), lr=0.004) #, betas=(0.9, 0.999))   #建立优化器
    log = []
...
if __name__ == "__main__":
    main()
    test_strings = ["abcceefd", "yrwfrger", "拼音weqgxx", "acnlhdww"]
